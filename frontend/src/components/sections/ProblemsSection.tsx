      ...
      {problems.map((problem: any, i: number) => (
        <div key={i} className="rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-4 space-y-3">
            ...
            {problem.hint && (
              <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-[10px] text-amber-300">
                <strong>Hint:</strong> {problem.hint}
              </div>
            )}
            {problem.solution && (
              <div className="rounded-lg border border-border overflow-hidden">
                <div className="px-3 py-1.5 bg-surface-150 border-b border-border text-[10px] text-text-tertiary font-mono">Solution</div>
                <pre className="p-3 text-[10px] font-mono leading-relaxed text-text-secondary overflow-x-auto"><code>{problem.solution}</code></pre>
              </div>
            )}
          </div>
        </div>
