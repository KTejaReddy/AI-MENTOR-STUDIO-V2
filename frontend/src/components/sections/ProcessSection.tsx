      ...
      {processes.map((proc: any, i: number) => (
        <div key={i} className="p-4 rounded-xl bg-surface-150 border border-border">
          <div className="flex items-center gap-2 mb-2">
            ...
            {proc.complexity && <span className="ml-auto text-xs font-mono text-accent-light">{proc.complexity}</span>}
          </div>
          {(proc.steps ?? []).length > 0 && (
            <ol className="space-y-1">
              {(proc.steps ?? []).map((s: string, j: number) => (
                <li key={j} className="flex items-start gap-2 text-xs text-text-secondary">
                  <span className="w-4 h-4 rounded bg-accent/10 flex items-center justify-center text-xs text-accent-light shrink-0 mt-0.5">{j + 1}</span>
                  {s}
                </li>
              ))}
            </ol>
