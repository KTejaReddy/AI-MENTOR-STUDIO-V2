...
export function TopNavbar({ onToggleSidebar, onToggleChat, chatOpen, onNewLesson }: TopNavbarProps) {
  ...
  return (
    <header className="mx-3 mt-3 h-14 shrink-0 glass rounded-2xl flex items-center justify-between px-4 z-40 shadow-lg relative border border-white/5">
      ...
      <div className="flex items-center gap-2">
        ...
        {isAuthenticated && user ? (
          <DropdownMenu
            ...
            trigger={
              <button
                className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-accent to-teal text-white text-xs font-bold shadow-md hover:shadow-glow-sm transition-all duration-300 focus:outline-none"
                title="Profile"
              >
                {user.avatar_url ? (
          ...
          <Tooltip content="Sign in">
            <IconButton label="Sign in" onClick={() => navigate('/login')}>
